...
3. Acknowledge: "I have the ethglossary skill loaded and I'm oriented on the repo."
4. Wait for direction before acting.
5. Never commit, push, or deploy without explicit single-use approval.
6. If asked to do something that touches glossary data, the `script_rule` enum, the `category` field, or translation policy: load the relevant reference file before editing.
