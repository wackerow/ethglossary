...
- **English style guide** -- 532 terms with casing rules, avoid lists, aliases, editorial notes. Authoritative for "what is the right way to write `<term>`?"
...
└── src/
    ...
    │   ├── styles.ts                # the stylesheet served at /assets/app.css
    │   ├── font-faces.ts            # GENERATED -- run pnpm run build:fonts
...
mounted by `src/routes/viewer.tsx`. There is no build step and no client
framework; interactivity is a handful of vanilla-JS islands in
`src/ui/islands.ts`.
...
- **Every color goes through a token.** The palette is defined three times in
  `src/ui/styles.ts`: bare `:root` (dark, the base), a
  `prefers-color-scheme: light` block guarded with `:root:not([data-theme="dark"])`,
  and `:root[data-theme="light"]`. A color declared in only one of those
...
### Rebuild the font subsets
```bash
pnpm run build:fonts
```

Regenerates `src/ui/font-faces.ts` and repopulates `public/fonts/` from the
`@fontsource` packages. Required after changing which scripts or weights the
viewer serves. Fonts are self-hosted -- never link a font CDN.
