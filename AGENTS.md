...
`src/ui/app.css`. There is no client framework; interactivity is a handful of
vanilla-JS islands in `src/ui/islands.ts`.
...
- **Use the theme scale, not arbitrary values.** `@theme` in `app.css` names
  ...
  `text-label-md`, `bg-surface`, `text-ink-dim`. Reach for `text-[19px]` only
  when the design genuinely has no token for it.
- **The palette is defined three times** in `app.css`: `@theme` (dark, the
  base), a `prefers-color-scheme: light` block guarded with
  `:root:not([data-theme="dark"])`, and `:root[data-theme="light"]`. Utilities
  compile to `var(--color-*)`, so redefining the variables is all a theme
  needs -- do NOT add `dark:` variants. A color defined in only one of the
  three renders one theme's text on the other theme's ground.
